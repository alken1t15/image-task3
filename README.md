...
- последовательные тесты дополнительно перебирают разные размеры изображений/ядер и проверяют композицию противоположных shift-фильтров до `identity`;
- параллельные тесты сравнивают все стратегии с последовательной реализацией на случайных данных, разных размерах, фильтрах, медианных окнах и числе потоков;
- batch-тесты сравнивают результат pipeline с последовательной свёрткой для каждого файла, проверяют сохранение относительных путей, ограниченные очереди и игнорирование неподдерживаемых файлов.
